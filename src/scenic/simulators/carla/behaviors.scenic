...
behavior CrosswalkBehavior(ref_actor, speed=1.4, threshold=10):   
    """Behavior for a pedestrian to cross a road at a crosswalk.
    Pedestrian must be placed at either end or start of the crosswalk.
    Args:
        ref_actor (Actor): The actor to synchronize with.
        speed (float): The speed of the pedestrian.
        threshold (float): The distance at which the pedestrian starts moving.
    """
    while (distance from self to ref_actor) > threshold:
        wait
